...
if ( ! class_exists( 'Zota_WooCommerce' ) ) :
	...
	class Zota_WooCommerce {
		...
		public function includes() {
			require_once( get_parent_theme_file_path( ZOTA_VENDORS . '/woocommerce/classes/class-wc-shop.php') );
			require_once( get_parent_theme_file_path( ZOTA_VENDORS . '/woocommerce/classes/class-wc-single.php') );
			require_once( get_parent_theme_file_path( ZOTA_VENDORS . '/woocommerce/classes/class-wc-cart.php') );
		}
		...
		private function init_hooks() {
			...
			if ( zota_tbay_get_config('enable_quickview', true) ) {
			    add_action( 'wp_ajax_zota_quickview_product', array( $this, 'quick_view_ajax'), 10 );
			    add_action( 'wp_ajax_nopriv_zota_quickview_product', array( $this, 'quick_view_ajax'), 10 );
			}
		...
		public function woocommerce_scripts() {
	        ...
	        wp_enqueue_script( 'zota-woocommerce', ZOTA_SCRIPTS . '/woocommerce' . $suffix . '.js', array( 'zota-script' ), ZOTA_THEME_VERSION, true );

	        wp_register_script( 'jquery-onepagenav', ZOTA_SCRIPTS . '/jquery.onepagenav' . $suffix . '.js', array( 'zota-script' ), '3.0.0', true ); 
		}
		...
		public function remove_wc_breadcrumb() {
			if( !zota_tbay_get_config('show_product_breadcrumb', false) ) {
		        remove_action( 'zota_woo_template_main_before', 'woocommerce_breadcrumb', 20, 0 );
		    } 
		}
		...
		public function wvs_theme_support() {
	        ...
	        if( class_exists( 'Woo_Variation_Swatches_Pro' ) ) {
	            ...
	            add_filter( 'woo_variation_swatches_archive_add_to_cart_text', function () {
	                return '<i class="tb-icon tb-icon-shopping-bag"></i><span class="title-cart">' . esc_html__( 'Add to cart', 'zota' ). '</span>';
	            } );

	            add_filter( 'woo_variation_swatches_archive_add_to_cart_select_options', function () {
	                return '<i class="tb-icon tb-icon-shopping-bag"></i><span class="title-cart">' . esc_html__( 'Select options', 'zota' ) . '</span>';
	            } );   
