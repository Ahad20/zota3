<?php
/*
  Plugin Name: BEAR - Bulk Editor and Products Manager
...
//***
        ...
        add_action('admin_menu', function() {
            add_submenu_page('edit.php?post_type=product', 'BEAR ' . __('Bulk Editor', 'woo-bulk-editor'), 'BEAR ' . __('Bulk Editor', 'woo-bulk-editor'), 'manage_woocommerce', 'woobe', function() {
        ...
        add_action('admin_bar_menu', function($wp_admin_bar) {
            ...
            if ($show) {
                $args = array(
                    'id' => 'woobe-btn',
                    'title' => 'BEAR ' . __('Bulk Editor', 'woo-bulk-editor'),
...
//***
    ...
    public function plugin_action_links($links) {
        ...
        $buttons = array(
            ...
            '<a target="_blank" href="https://bulk-editor.com/"><span class="icon-book"></span>&nbsp;' . __('Documentation', 'woo-bulk-editor') . '</a>'
        );
        ...
        if (isset($_GET['page']) AND $_GET['page'] == 'woobe') {
            ...
            <script>
                ...
                var woobe_settings = {
                    ...
                    autocomplete_max_elem_count: 10
                };
    ...
    public function ask_favour() {
        ...
        add_action('admin_notices', function() use($slug) {
            ...
            <div class="notice notice-info" id="pn_<?php echo $slug ?>_ask_favour" style="position: relative;">
                <button onclick="javascript: pn_<?php echo $slug ?>_dismiss_review(1); void(0);" title="<?php _e('Later', 'woo-bulk-editor'); ?>" class="notice-dismiss"></button>
                <div id="pn_<?php echo $slug ?>_review_suggestion">
                    <p><?php _e('Hi! Are you enjoying using <i>BEAR - WooCommerce Bulk Editor and Products Manager Professional</i>?', 'woo-bulk-editor'); ?></p>
                    <p><a href="javascript: pn_<?php echo $slug ?>_set_review(1); void(0);"><?php _e('Yes, I love it', 'woo-bulk-editor'); ?></a> 🙂 | <a href="javascript: pn_<?php echo $slug ?>_set_review(0); void(0);"><?php _e('Not really...', 'woo-bulk-editor'); ?></a></p>
                </div>

                <div id="pn_<?php echo $slug ?>_review_yes" style="display: none;">
                    <p><?php printf(__('That\'s awesome! Could you please do us a BIG favor and give it a 5-star rating on %s to help us spread the word and boost our motivation?', 'woo-bulk-editor'), $on) ?></p>
                ...
                <div id="pn_<?php echo $slug ?>_review_no" style="display: none;">
                    <p><?php _e('We are sorry to hear you aren\'t enjoying BEAR. We would love a chance to improve it. Could you take a minute and let us know what we can do better?', 'woo-bulk-editor'); ?></p>
                    <p>
...
function woobe_init_translates() {
    try {
        ...
        //$locale = apply_filters('plugin_locale', determine_locale(), $lang_domain);
        $locale = get_locale();
